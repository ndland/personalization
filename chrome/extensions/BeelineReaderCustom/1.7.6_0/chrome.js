...
(function() {
  ...
  window.BeeLineReader = (function() {
    ...
    BeeLineReader.DEFAULT_THEME = 'custom';
    ...
    BeeLineReader.prototype.color = function(callback) {
      ...
      if (this.getTheme() === 'custom') {
        return this._traverse.apply(this, [this.node].concat(__slice.call(this._customColors()), [callbackWithTrigger]));
      } else {
        return this._traverse.apply(this, [this.node].concat(__slice.call(BeeLineReader.THEMES[this.getTheme()]), [callbackWithTrigger]));
      }
    };

    BeeLineReader.prototype.wrap = function(callback) {
      if (!this.browserIsCompatible()) {
        return typeof callback === "function" ? callback() : void 0;
      }
      if (this.getTheme() === 'custom') {
        return this._traverse.apply(this, [this.node].concat(__slice.call(this._customColors()), [callback], [{
          wrapOnly: true
        }]));
      } else {
        return this._traverse.apply(this, [this.node].concat(__slice.call(BeeLineReader.THEMES[this.getTheme()]), [callback], [{
          wrapOnly: true
        }]));
      }
    };
    ...
    BeeLineReader.prototype._customColors = function() {
      return [this.options.customColor1 || BeeLineReader.THEMES['custom'][0], this.options.customColor2 || BeeLineReader.THEMES['custom'][1], this.options.customColor3 || BeeLineReader.THEMES['custom'][2], this.options.customColor4 || BeeLineReader.THEMES['custom'][3], this.options.customBackground || BeeLineReader.THEMES['custom'][4]];
    };
    ...
    BeeLineReader.prototype._traverse = function(parseNode, firstColorMinHex, firstColorMaxHex, secondColorMinHex, secondColorMaxHex, backgroundColorHex, callback, opts) {
      var _this = this;
      if (opts == null) {
        opts = {};
      }
      ...
      return BeelineUtils.whenTrue((function() {
        ...
        var onTraverseCompletion, state;
        onTraverseCompletion = function() {
        ...
        if (parseNode.hasChildNodes()) {
          ...
          state = {
            ...
            colorPairs: [],
            wrapOnly: opts.wrapOnly
          };
          if (state.xmin === state.xmax) {
            state.xmax += 1;
          }
          _this._wrapAndColor(parseNode, state);
          return _this._workOff(state.workQueue1, state.workQueue2, {
            abortable: true,
            after: function(aborted) {
              if (aborted) {
                _this.trigger('coloringAborted');
                BeelineUtils.log("aborted coloring");
                return onTraverseCompletion();
              } else {
                _this._colorLine(state, true);
                return _this._workOff(state.workQueue1, state.workQueue2, {
                  after: onTraverseCompletion
                });
              }
            }
          });
    ...
    BeeLineReader.prototype._wrapAndColor = function(node, state) {
      ...
      if (node.nodeType === 1) {
          ...
          while (true) {
            ...
            if (!(node.nodeType === 1 && ((_ref = node.nodeName.toLowerCase(), __indexOf.call(this.options.skipTags, _ref) >= 0) || node.getAttribute(BeeLineReader.SKIP_COLOR_ATTRIBUTE) === 'true'))) {
              if (node.className === this.BEELINE_INDICATOR_CLASS) {
                if (!state.wrapOnly) {
                  this._colorNode(node, state);
                }
              } else {
                this._wrapAndColor(node, state);
              }
            }
            if (!(node = next)) {
              break;
        ...
        return state.workQueue1.push(function() {
            ...
            for (j = _i = 0; 0 <= numchars ? _i < numchars : _i > numchars; j = 0 <= numchars ? ++_i : --_i) {
              ...
              if (thisChar === ' ' || thisChar === "\n" || thisChar === "\t" || thisChar === "\r") {
                ...
                if (gatheredChars) {
                  ...
                  if (state.wrapOnly) {
                    _this._measureNode(newSpan, state);
                  } else {
                    _this._colorNode(newSpan, state);
                  }
                }
                gatheredChars = thisChar;
              }
    ...
    BeeLineReader.prototype._measureNode = function(node, state) {
      var _this = this;
      return state.workQueue2.push(function() {
        if (state.reflowNeeded) {
          return node.cachedBeeLineXRect = node.getBoundingClientRect();
        } else {
          return node.cachedBeeLineXRect || (node.cachedBeeLineXRect = node.getBoundingClientRect());
        }
      });
    };

    BeeLineReader.prototype._colorNode = function(node, state) {
      var _this = this;
      return state.workQueue2.push(function() {
    ...
    BeeLineReader.prototype._workOff = function(queue1, queue2, opts) {
      var fn, startTime,
        _this = this;
      if (opts == null) {
        opts = {};
      ...
      while ((new Date()).getTime() - startTime < this.workInterval && (fn = queue1.shift() || queue2.shift())) {
        fn();
      }
      if (queue1.length + queue2.length > 0 && opts.abortable && this.abortCount > 0) {
        if (opts.after) {
          return opts.after(true);
        }
      } else if (queue1.length + queue2.length > 0) {
        return BeelineUtils.afterRender(function() {
          return _this._workOff(queue1, queue2, opts);
        });
      } else {
        if (opts.after) {
          return opts.after();
    ...
    return window.BeeLineReaderExtension = (function() {
        ...
        return this.loadSettings(function() {
          ...
          if (_this.shouldInlineColor()) {
            ...
            return _this.launchDelay(function() {
              var _ref, _ref1;
              if (_this.autoUsageAllowed()) {
                if (((_ref = _this.options.customSite) != null ? (_ref1 = _ref.disable) != null ? _ref1.backgrounds : void 0 : void 0) && BeelineUtils.colorIsDark(_this.backgroundColor())) {
                  _this.options.theme = "bright";
                }
                _this.options.customSite.run();
                _this.disabled = !_this.options.customSite.working;
                if (!_this.disabled) {
                  _this.gaTrackLoad();
                  return _this.sharedStartupActions();
                }
              } else {
                _this.trackAutoDisallow();
                return _this.sharedStartupActions();
              }
            });
          } else {
      ...
      BeeLineReaderExtension.prototype.checkForCustomSite = function() {
        ...
        for (className in _ref) {
          ...
          if (handler.matches() && (!this.options.legacy || handler.legacy)) {
      ...
      BeeLineReaderExtension.prototype.trackAutoDisallow = function() {
        var now, _base;
      ...
      BeeLineReaderExtension.prototype._showHelpOnUpgrade = function() {
        if (!this.options.hasShownHelp || this.options.hasShownHelp < this.version) {
          ...
          if (!this.options.kioskMode) {
            return this._attachPopup(this.helpMessage());
          }
        }
      };
      ...
      BeeLineReaderExtension.prototype.addBeeline = function(beeline) {
      ...
      BeeLineReaderExtension.prototype.autoUsageAllowed = function() {
        var autoUsageAllowed, _ref;
        if (!this.freePlan()) {
          return true;
        }
        this.cleanupOldColoringData();
        autoUsageAllowed = this.options.usageData.auto.sites[(_ref = this.options.customSite) != null ? _ref.name : void 0] || this.options.usageData.auto.count < 5;
      ...
      BeeLineReaderExtension.prototype.cleanupOldColoringData = function() {
        var midnight, midnightDate, minutes, now, siteName, timestamp, _ref, _ref1, _results;
        now = (new Date()).getTime();
        midnightDate = new Date();
        midnightDate.setHours(0, 0, 0, 0);
        midnight = midnightDate.getTime();
        if (this.development()) {
          BeelineUtils.log("Midnight would be " + midnight);
        }
        ...
        for (siteName in _ref) {
          timestamp = _ref[siteName];
          if (((_ref1 = this.options.customSite) != null ? _ref1.name : void 0) === siteName && !this.options.customSite.singlePage) {
            minutes = 2;
          } else {
            minutes = 15;
          }
          if ((timestamp || 0) < now - minutes * 60 * 1000) {
            if (this.development()) {
              BeelineUtils.log("last visit to '" + siteName + "' was > " + minutes + " min ago");
            }
      ...
      BeeLineReaderExtension.prototype.recordColoring = function() {
        var now,
          _this = this;
        if (this.options.kioskMode) {
            ...
            return this.getVariables('usageData', function(result) {
              ...
              if (now > (_this.lastColorRecording || 0) + 15000) {
                ...
                if (!_this.shouldAutoColor()) {
                  ...
                  if (!_this.options.usageData.auto.sites[_this.options.customSite.name]) {
                    _this.options.usageData.auto.sites[_this.options.customSite.name] = now;
                    _this.options.usageData.auto.count += 1;
                    _this.options.usageData.auto.lastIncremented = now;
                    changed = true;
                    if (_this.development()) {
                      BeelineUtils.log("On " + _this.options.customSite.name + ", incremented auto count to " + _this.options.usageData.auto.count);
                    }
          ...
          switch (cmd) {
            case "set":
              ...
              object[args[0]] = (function() {
                try {
                  return JSON.parse(args.slice(1, args.length).join(' '));
                } catch (_error) {
...
(function() {
  ...
  (function($) {
    var _ref;
    return window.BeeLineReaderChrome = (function(_super) {
      __extends(BeeLineReaderChrome, _super);

      function BeeLineReaderChrome() {
        this.setVariablesInStorage = __bind(this.setVariablesInStorage, this);
        this.getVariablesFromStorage = __bind(this.getVariablesFromStorage, this);
        _ref = BeeLineReaderChrome.__super__.constructor.apply(this, arguments);
        return _ref;
      }

      BeeLineReaderChrome.prototype.EXTENSION_NAME = 'Chrome';

      BeeLineReaderChrome.prototype.getVariablesFromStorage = function(variables, callback) {
        var _this = this;
        return chrome.storage.sync.get(variables, function(answers) {
          if (chrome.runtime.lastError) {
            _this.gaTrack("sync.get/" + (encodeURIComponent(chrome.runtime.lastError.message)), 'errors');
          }
          return callback && callback(answers);
        });
      };

      BeeLineReaderChrome.prototype.setVariablesInStorage = function(variables, callback) {
        var _this = this;
        return chrome.storage.sync.set(variables, function() {
          if (chrome.runtime.lastError) {
            _this.gaTrack("sync.set/" + (encodeURIComponent(chrome.runtime.lastError.message)), 'errors');
          }
          return callback && callback();
        });
      };

      BeeLineReaderChrome.prototype.introHelpMessage = function() {
        return "<p>Welcome to the BeeLine Chrome extension, v1.7.6!</p>";
      };

      BeeLineReaderChrome.prototype.keyboardHelpMessage = function() {
        return "add a keyboard shortcut in the Extensions manager</p>";
      };

      BeeLineReaderChrome.prototype.loadOpenDyslexic = function(doc) {
        var _ref1;
        if (doc == null) {
          doc = document;
        }
        return BeelineUtils.loadCSS(typeof chrome !== "undefined" && chrome !== null ? (_ref1 = chrome.extension) != null ? _ref1.getURL("vendor/open-dyslexic.css") : void 0 : void 0, {
          id: 'open-dyslexic-link',
          document: doc
        });
      };

      BeeLineReaderChrome.prototype.loadResources = function(options) {
        var _ref1, _ref2, _ref3, _ref4, _ref5;
        if (options == null) {
          options = {};
        }
        BeelineUtils.loadCSS(typeof chrome !== "undefined" && chrome !== null ? (_ref1 = chrome.extension) != null ? _ref1.getURL("vendor/spectrum.css") : void 0 : void 0);
        BeelineUtils.loadCSS(typeof chrome !== "undefined" && chrome !== null ? (_ref2 = chrome.extension) != null ? _ref2.getURL("chrome.css") : void 0 : void 0);
        BeelineUtils.loadCSS(typeof chrome !== "undefined" && chrome !== null ? (_ref3 = chrome.extension) != null ? _ref3.getURL("vendor/magnific-popup/magnific-popup.css") : void 0 : void 0);
        if (!((_ref4 = options.disable) != null ? _ref4.openDyslexic : void 0)) {
          this.loadOpenDyslexic();
        }
        if (options.readability != null) {
          return BeelineUtils.loadCSS(typeof chrome !== "undefined" && chrome !== null ? (_ref5 = chrome.extension) != null ? _ref5.getURL("vendor/readability_print.css") : void 0 : void 0, {
            media: 'print'
          });
        }
      };

      return BeeLineReaderChrome;
...
(function() {
  ...
  BeeLineReaderExtension.customHandlers.AmazonHandler = (function(_super) {
    ...
    AmazonHandler.prototype.singlePage = true;
...
(function() {
  ...
  BeeLineReaderExtension.customHandlers.AolHandler = (function(_super) {
    ...
    AolHandler.prototype.singlePage = true;
...
(function() {
  ...
  BeeLineReaderExtension.customHandlers.ApexLearningHandler = (function(_super) {
    ...
    ApexLearningHandler.prototype.singlePage = true;
...
(function() {
  ...
  BeeLineReaderExtension.customHandlers.CNETHandler = (function(_super) {
    __extends(CNETHandler, _super);

    function CNETHandler() {
      _ref = CNETHandler.__super__.constructor.apply(this, arguments);
      return _ref;
    }

    CNETHandler.prototype.pattern = /^https?:\/\/(\w+\.)?cnet\.com(\.\w{2})?/i;

    CNETHandler.prototype.name = 'CNET';

    CNETHandler.prototype.positions = {
      toolsMenu: {
        left: "0px",
        right: "auto",
        top: "0px"
    ...
    CNETHandler.prototype.run = function() {
      var _this = this;
      this.skip(".meta, .hotness, .btn-link, .col-3.infoContainer, .productPrice, .clickToEnlarge, .videoPromoContainer, .toggleComments, .caption, .pullQuote, .related-links");
      this.keepColoringSelector(".col-8, .description, #editorReview");
      return setTimeout((function() {
...
(function() {
  ...
  BeeLineReaderExtension.customHandlers.DevelopmentHandler = (function(_super) {
    __extends(DevelopmentHandler, _super);

    function DevelopmentHandler() {
      _ref = DevelopmentHandler.__super__.constructor.apply(this, arguments);
      return _ref;
    }

    DevelopmentHandler.prototype.pattern = /^https?:\/\/localhost:8001/i;

    DevelopmentHandler.prototype.name = 'Development';

    DevelopmentHandler.prototype.legacy = true;

    DevelopmentHandler.prototype.singlePage = true;

    DevelopmentHandler.prototype.autoColor = function() {
      return this.extension.development();
    };

    DevelopmentHandler.prototype.run = function() {
      return this.keepColoringSelector("p");
    };
...
(function() {
  ...
  BeeLineReaderExtension.customHandlers.ESPNHandler = (function(_super) {
    ...
    ESPNHandler.prototype.singlePage = true;
...
(function() {
  ...
  BeeLineReaderExtension.customHandlers.FacebookHandler = (function(_super) {
    ...
    FacebookHandler.prototype.singlePage = true;
...
(function() {
  ...
  BeeLineReaderExtension.customHandlers.FeedlyHandler = (function(_super) {
    ...
    FeedlyHandler.prototype.singlePage = true;
...
(function() {
  ...
  (function($) {
    ...
    return BeeLineReaderExtension.customHandlers.GMAILHandler = (function(_super) {
      ...
      GMAILHandler.prototype.singlePage = true;
...
(function() {
  ...
  (function($) {
    ...
    return BeeLineReaderExtension.customHandlers.GoogleDocsHandler = (function(_super) {
      ...
      GoogleDocsHandler.prototype.singlePage = true;
...
(function() {
  ...
  BeeLineReaderExtension.customHandlers.GoogleNewsHandler = (function(_super) {
    ...
    GoogleNewsHandler.prototype.singlePage = true;
...
(function() {
  ...
  (function($) {
    ...
    return BeeLineReaderExtension.customHandlers.KindleReaderHandler = (function(_super) {
      ...
      KindleReaderHandler.prototype.singlePage = true;
...
(function() {
  ...
  BeeLineReaderExtension.customHandlers.LinkedInHandler = (function(_super) {
    ...
    LinkedInHandler.prototype.singlePage = true;
...
(function() {
  ...
  BeeLineReaderExtension.customHandlers.OpenStaxHandler = (function(_super) {
    ...
    OpenStaxHandler.prototype.singlePage = true;
...
(function() {
  ...
  (function($) {
    ...
    return BeeLineReaderExtension.customHandlers.OverdriveHandler = (function(_super) {
      ...
      OverdriveHandler.prototype.singlePage = true;
...
(function() {
  ...
  BeeLineReaderExtension.customHandlers.OysterHandler = (function(_super) {
    ...
    OysterHandler.prototype.singlePage = true;
...
(function() {
  ...
  BeeLineReaderExtension.customHandlers.PocketHandler = (function(_super) {
    ...
    PocketHandler.prototype.singlePage = true;
...
(function() {
  ...
  BeeLineReaderExtension.customHandlers.TumblrHandler = (function(_super) {
    ...
    TumblrHandler.prototype.singlePage = true;
...
(function() {
  ...
  (function($) {
    ...
    return BeeLineReaderExtension.customHandlers.WordpressYoastHandler = (function(_super) {
      ...
      WordpressYoastHandler.prototype.singlePage = true;
